...
export function CalibrationTab({ mode, onModeChange }: CalibrationTabProps) {
  ...
  const handleCreateProfile = async (
    colorAdjustments: Array<{ color: string, swatchIndex: number, adjustment: { r: number, g: number, b: number } }>,
    profileName: string,
    deviceName: string
  ) => {
    const adjustmentsRecord: Record<string, { r: number; g: number; b: number }> = {}
    colorAdjustments.forEach(({ color, swatchIndex, adjustment }) => {
      adjustmentsRecord[createProfileAdjustmentKey(color, swatchIndex)] = adjustment
    })
  ...
  const handleUseExportAsScan = async () => {
    try {
      const pngDataUrl = await renderColorChartToPngDataUrl(cardLayout, colorChart, useArucoMarkers)
      onModeChange('compare')
      window.requestAnimationFrame(() => {
        handleImageData(pngDataUrl, colorChart, cardLayout)
      })
    } catch (error) {
      console.error('Failed to use calibration export as scan:', error)
      alert('Failed to use the current chart export as a scan.')
    }
  }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, minWidth: 0 }}>
          ...
          <div style={{ background: 'var(--bg-card)', borderRadius: 8, border: '1px solid var(--border)', padding: 14, flex: 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, marginBottom: 10, flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', justifyContent: 'flex-end', maxWidth: 620 }}>
                <select
                ...
                <button className="btn btn-secondary btn-sm" onClick={() => void generatePNG(cardLayout, colorChart, useArucoMarkers)}>
                ...
                <button className="btn btn-secondary btn-sm" onClick={() => void handleUseExportAsScan()}>
                  <ScanLine size={14} style={{ marginRight: 4 }} />
                  Scan Export
                </button>
              </div>
            </div>
        ...
        <div style={{ background: 'var(--bg-card)', borderRadius: 8, border: '1px solid var(--border)', padding: 14, display: 'flex', flexDirection: 'column', position: scannedImage ? 'sticky' : 'static', top: 12 }}>
          ...
          {!scannedImage && (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', borderTop: '1px solid var(--border)', paddingTop: 24 }}>
              ...
              <button className="btn btn-secondary" onClick={() => void handleUseExportAsScan()} style={{ marginTop: 8 }}>
              ...
              <div style={{ flex: 1, overflow: 'auto', padding: 14 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(118px, 1fr))', gap: 8, width: '100%' }}>
                  {Object.entries(selectedProfile.adjustments || {}).map(([color, adjustment]) => {
                    ...
                    return (
                      <div key={color} style={{ background: 'var(--bg-hover)', borderRadius: 6, padding: 8 }}>
